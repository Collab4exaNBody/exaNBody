...
the ghost cells are always empty after this operator.
    ...
    inline void execute () override final
    {
      ...
      */
      ...
      MigrateCellParticlesImpl<GridT,CellValueMergeOperatorT>::migrate_cell_particles(
          ldbg
        ...
        , *domain
        , *grid
        , *ghost_dist //max_nbh_dist
        , grid_cell_values_ptr
        );
      
      if( ! otb_particles->empty() )
      {
        ldbg << otb_particles->size() << " particles left in otb_particles" << std::endl;
        otb_particles->clear();
      }
    }
