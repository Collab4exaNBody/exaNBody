...
namespace exanb
{
  ...
  struct ComputeCellParticlesFunctor2< CellsT, FuncT, FieldAccTupleT , std::index_sequence<FieldIndex...> >
  {
...
namespace onika
{
  namespace parallel
  {
    ...
    struct BlockParallelForFunctorTraits< exanb::ComputeCellParticlesFunctor2<CellsT,FuncT,FieldTupleT,IndexSequenceT> >
    {
...
namespace exanb
{
    ...
    int size_cells )
  {
    ...
    using PForFuncT = ComputeCellParticlesFunctor2<CellsAccessorT,FuncT,FieldTupleT,std::make_index_sequence<sizeof...(FieldAccT)> >;
    
    const IJK dims = grid.dimension();
    const int gl = enable_ghosts ? 0 : grid.ghost_layers();
    const IJK block_dims = dims - (2*gl);
    const size_t N = /*block_dims.i * block_dims.j * block_dims.k*/size_cells;    
    ...
    int size_cells )
  {
    ...
    return compute_cell_particles2(grid,enable_ghosts,func,cp_fields,exec_ctx, cells_non_empty, size_cells);
  }
