...
function(GenerateSourcesFromInput src srcdirlistdd dstdir VariantDB outsrclist outtarget)
...
#    message(STATUS "replace ${srcdirlistdd} -> ${dstdir} => ${${VariantDB}SourceFilePrefix}")
    ...
    add_custom_target(${TargetName} COMMAND ${CMAKE_COMMAND} -DSourceFile=${src} -DInputBaseDir="${srcdirlistdd}" -DOutputBaseDir=${PROJECT_BINARY_DIR} ${tmpvardef} -P ${CONFIGURE_GENERATE_VARIANTS_SCRIPT} DEPENDS ${src})
    add_custom_command(OUTPUT ${${VariantDB}SourceFiles} COMMAND ${CMAKE_COMMAND} -DSourceFile=${src} -DInputBaseDir="${srcdirlistdd}" -DOutputBaseDir=${PROJECT_BINARY_DIR} ${tmpvardef} -P ${CONFIGURE_GENERATE_VARIANTS_SCRIPT} DEPENDS ${src})
...
#      message(STATUS "process ${src} to DataBase ${VariantDB}")
      GenerateSourcesFromInput(${src} "${PROJECT_SOURCE_DIR}:${XNB_ROOT_DIR}" ${PROJECT_BINARY_DIR} ${VariantDB} ${outsrclist} InputTargetName)
...
function(GenerateDirectorySourceFilesFunc dir var outtarget)
  ...
  foreach(srcfile ${sources})
    ...
    if("${srcdata}" MATCHES "#[ \t]*pragma[ \t]+xstamp_cuda_enable" AND ONIKA_BUILD_CUDA)
...
set_property(GLOBAL PROPERTY GLOBAL_ONIKA_CMAKE_SOURCE_ID 0)
set_property(GLOBAL PROPERTY GLOBAL_ONIKA_CMAKE_TARGET_ID 0)
set(ONIKA_CMAKE_SOURCE_ID 0)
set(ONIKA_CMAKE_TARGET_ID 0)
set(ONIKA_CMAKE_TARGET_ID_DEFINITIONS -DONIKA_CMAKE_TARGET_ID=${ONIKA_CMAKE_TARGET_ID})

macro(XstampIncrementTargetId)
  get_property(ONIKA_CMAKE_TARGET_ID GLOBAL PROPERTY GLOBAL_ONIKA_CMAKE_TARGET_ID)
  math(EXPR ONIKA_CMAKE_TARGET_ID ${ONIKA_CMAKE_TARGET_ID}+1)
  set_property(GLOBAL PROPERTY GLOBAL_ONIKA_CMAKE_TARGET_ID ${ONIKA_CMAKE_TARGET_ID})
  set(ONIKA_CMAKE_TARGET_ID_DEFINITIONS -DONIKA_CMAKE_TARGET_ID=${ONIKA_CMAKE_TARGET_ID})
endmacro()

macro(XstampIncrementSourceId)
  get_property(ONIKA_CMAKE_SOURCE_ID GLOBAL PROPERTY GLOBAL_ONIKA_CMAKE_SOURCE_ID)
  math(EXPR ONIKA_CMAKE_SOURCE_ID ${ONIKA_CMAKE_SOURCE_ID}+1)
  set_property(GLOBAL PROPERTY GLOBAL_ONIKA_CMAKE_SOURCE_ID ${ONIKA_CMAKE_SOURCE_ID})
endmacro()
...
macro(GenerateDirectorySourceFiles dir var outtarget)
  ...
  foreach(src ${${var}})
    ...
    set_source_files_properties(${src} PROPERTIES COMPILE_DEFINITIONS ONIKA_CMAKE_SOURCE_ID=${ONIKA_CMAKE_SOURCE_ID})
  endforeach()
  if(${var}_CU)
    foreach(src ${${var}_CU})
      XstampIncrementSourceId()
#      message(STATUS "file ${src} : ONIKA_CMAKE_SOURCE_ID=${ONIKA_CMAKE_SOURCE_ID}")
      set_source_files_properties(${src} PROPERTIES COMPILE_DEFINITIONS ONIKA_CMAKE_SOURCE_ID=${ONIKA_CMAKE_SOURCE_ID})
    endforeach()
  endif()
endmacro()
