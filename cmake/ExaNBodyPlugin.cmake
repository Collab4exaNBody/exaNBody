...
#    endif()
    ...
    if(${PluginName}_SCRS_LIB)
      ...
      target_compile_definitions(${PluginName} PRIVATE ${XNB_COMPILE_DEFINITIONS} PUBLIC ${${PluginName}_COMPILE_DEFINITIONS})
      target_include_directories(${PluginName} PRIVATE ${dirname}/lib ${XNB_INCLUDE_DIRS} PUBLIC ${${PluginName}_INCLUDE_DIRS} ${dirname}/include)
      target_link_directories(${PluginName} PUBLIC ${XNB_LIBRARY_DIRS} ${${PluginName}_LINK_DIRECTORIES})
...
#    message(STATUS "Plugin ${PluginName}: HOST ${${PluginName}_SCRS} , target=${${PluginName}_DEPS} CUDA ${${PluginName}_SCRS_CU} , target=${${PluginName}_DEPS_CU}")
    ...
    if(${PluginName}_SCRS_CU)
      if(ONIKA_BUILD_CUDA)
        ...
        target_include_directories(${PluginName}Plugin PRIVATE ${CUDA_SAMPLES_INCLUDE_DIR} ${dirname} ${XNB_INCLUDE_DIRS} ${${PluginName}_INCLUDE_DIRS})
        target_compile_definitions(${PluginName}Plugin PRIVATE ${XNB_COMPILE_DEFINITIONS} ${${PluginName}_COMPILE_DEFINITIONS})        
        if(ONIKA_ENABLE_HIP)
        ...
        target_compile_features(${PluginName}Plugin PRIVATE ${ONIKA_COMPILE_FEATURES})
        target_link_directories(${PluginName}Plugin PRIVATE ${XNB_LIBRARY_DIRS} ${${PluginName}_LINK_DIRECTORIES})
    ...
    if(NOT ${PluginName}_HAS_CUDA AND ${PluginName}_SCRS)
      ...
      target_compile_definitions(${PluginName}Plugin PRIVATE ${XNB_COMPILE_DEFINITIONS} ${${PluginName}_COMPILE_DEFINITIONS})
      target_include_directories(${PluginName}Plugin PRIVATE ${dirname} ${XNB_INCLUDE_DIRS} ${${PluginName}_INCLUDE_DIRS})
      target_link_libraries(${PluginName}Plugin PRIVATE ${PLUGIN_CORE_LIBS} ${${PluginName}_LINK_LIBRARIES} ${LibTarget})
